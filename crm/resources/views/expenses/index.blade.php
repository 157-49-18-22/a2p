...
<div class="row">
    ...
    <div class="col-md-3">    
		...
		<div class="btn wallet-balance text-uppercase float-right" >
                <span class="big-btn-text">Wallet Balance : {{ App\Lancer\Utilities::CURRENCY_SYMBOL }} <?php 
				if($wallet >= 0) { echo $wallet ;} else { echo '0'; } ?></span>
		</div>
		@endif
		    </div>
</div>
            ...
            <table class="table">
                <thead class="thead-dark">
                    <tr>
                        ...
                        <th class="text-uppercase" scope="col">Emp ID</th>
                        <th class="text-uppercase" scope="col">Payee</th>
                        <th class="text-uppercase" scope="col">Location</th>
                ...
                <tbody>
                    ...
                    <tr>
                        <td>{{ $expense->id }}</td>
                        <td>A2P_{{ $expense->created_by }}</td>
                        <td>{{ $expense->payee }}</td>
						<td>{{ $expense->createdBy->location }}</td>
