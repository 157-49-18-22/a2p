...
if (isset($_POST['addclient'])) {
    ...
    $type = 1;
    $pdo = getPDOObject();
    // $sql = $pdo->query("SELECT * FROM `testimonial` where  name LIKE '$name'");
    // $num = $sql->rowCount();
    $num = 0;
    ...
    if (!$num) {
        ...
        $q = $pdo->prepare("INSERT into `testimonial` values(:id,:name, :photo,:des,:des1, :fld_order, :actstat)");
        $q->execute(array(
            ':id' => $id,
...
if ($_GET['action'] == 'delete') {
    $sql_del = sqlfetch("delete from testimonial where id='" . $_GET['id'] . "'");
...
if (isset($_POST['deleteall'])) {
    ...
    if (count($arr)) {
        ...
        $data = sqlfetch("select * from `testimonial` where id in ($str_rest_refs)");
        ...
        $pdo = getPDOObject();
        $q = $pdo->query("DELETE FROM `testimonial` WHERE id in ($str_rest_refs)");
        if ($q)
            $umessage = '<div class="alert alert-success" role="alert">
...
<div class="content-wrapper">
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <h4 class="py-3 mb-4"><span class="text-muted fw-light">Home/</span>Testimonial Settings</h4>
                    ...
                    <div class="card mb-4">
                        ...
                        <div class="card-body demo-vertical-spacing demo-only-element z4">
                            <div class="row">
                                <?php
                                $pid = $_GET['pid'];
                                $productdata = sqlfetch("SELECT * FROM `testimonial` where id='$pid' ");
                ...
                <div class="card-datatable dataTable_select text-nowrap table-responsive">
                    <table id="tableID" class="display dt-select-table table table-bordered" style="width:100%">
                        ...
                        <tbody>
                            <?php
                            $count = 1;
                            $data = sqlfetch("SELECT * FROM `testimonial`  order by fld_order");
                            foreach ($data as $menu) { ?>
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <a class="ajax-link" href="testimonial.php?&pid=<?php echo $menu['id']; ?>&edit=true">
                                    ...
                                    <td>
                                        ...
                                        <a href="testimonial.php?id=<?php echo $menu['id']; ?>&action=delete" onclick="return confirm('Are you sure you want to delete this item?');">
