...
if (isset($_POST['addsubproduct'])) {
    ...
    $photos3 = $_FILES['photo3']['name'];
    $Filename3 = '';

    $photos4 = $_FILES['photo4']['name'];
    $Filename4 = '';
    ...
    if (!$num) {
        ...
        if ($photos3) {
            $Filename3 = date('dmyhis') . basename($_FILES['photo3']['name']);
            $posted_data['photo3'] = $Filename3;
            $target = "../upload/" . $Filename3;
            move_uploaded_file($_FILES['photo3']['tmp_name'], $target);    //Tells you if its all ok	
        }

        if ($photos4) {
            $Filename4 = date('dmyhis') . basename($_FILES['photo4']['name']);
            $posted_data['photo4'] = $Filename4;
            $target = "../upload/" . $Filename4;
            move_uploaded_file($_FILES['photo4']['tmp_name'], $target);    //Tells you if its all ok	
        }
...
if ($_GET['action'] == 'delete') {
    $sql_del = sqlfetch("delete from subproduct where id='" . $_GET['id'] . "'");
...
if (isset($_POST['editdone'])) {
    ...
    if ($photos3 != '') {
        ...
        $target = "../upload/" . $Filename3;
        move_uploaded_file($_FILES['photo3']['tmp_name'], $target);    //Tells you if its all ok	
        $img_path = '../upload/' . $prevphoto3;
        if (file_exists($img_path)) {
            @unlink($img_path);
        }
    }

    $Filename4 = $prevphoto4;
    $photos4 = $_FILES['photo4']['name'];
    if ($photos4 != '') {
        $Filename4 = '';
        ...
        $target = "../upload/" . $Filename4;
        move_uploaded_file($_FILES['photo4']['tmp_name'], $target);    //Tells you if its all ok	
        $img_path = '../upload/' . $prevphoto4;
        if (file_exists($img_path)) {
            @unlink($img_path);
        }
    }

    $affected_rows = update('subproduct', $posted_data, array('id' => $pid));
    if ($affected_rows)
        $umessage = '<div class="alert alert-success" role="alert">
                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-12  mt-4 ">
                            <div class="input-group input-group-merge">
                                <span class="input-group-text">@</span>
                                <div class="form-floating form-floating-outline">
                                    <input type="text" class="form-control" name="pro_lable" placeholder="Name " aria-label="Name" value="<?php echo $pro_lable; ?>" />
                                    <label for="basic-addon11"> Product Location </label>
                                </div>
                            </div>
                        </div>
                ...
                <div class="col-lg-12  mt-4">
                    <div class="input-group input-group-merge">
                        ...
                        <div class="form-floating form-floating-outline">
                            ...
                            <br><br>
                            <textarea id="mytextarea" name="pro_additionalinfo" cols="60" rows="10"><?php echo $pro_additionalinfo; ?></textarea>
                        </div>
                    </div>
                </div>
...
<div class="content-wrapper">
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <?php } ?>
        <script>
            // Run the generateCode function when the page is loaded
