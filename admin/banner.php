...
if (isset($_POST['addclient'])) {
    ...
    if (!$num) {
        ...
        $q = $pdo->prepare("INSERT into `client` values(:id,:name, :photo,:des,:des1, :fld_order, :actstat)");
        $q->execute(array(
            ':id' => $id,
...
if ($_GET['action'] == 'delete') {
    $sql_del = sqlfetch("delete from client where id='" . $_GET['id'] . "'");
    echo "<script>window.open('banner.php','_self')</script>";
}
...
if (isset($_POST['deleteall'])) {
    ...
    if (count($arr)) {
        ...
        $data = sqlfetch("select * from `client` where id in ($str_rest_refs)");
        ...
        $pdo = getPDOObject();
        $q = $pdo->query("DELETE FROM `client` WHERE id in ($str_rest_refs)");
        if ($q)
            $umessage = '<div class="alert alert-success" role="alert">
...
if (isset($_POST['editdone'])) {
    ...
    $pdo = getPDOObject();
    $q = $pdo->prepare("UPDATE `client` SET 
            ...
            <div class="row">
                ...
                <div class="col-lg-4  mt-3">
                    <div class="input-group input-group-merge">
                        <span class="input-group-text">@</span>
                        <div class="form-floating form-floating-outline">
                            <input type="text" class="form-control" name="name" placeholder="Link" aria-label="Name" value="<?php echo $name; ?>" />
                            <label for="basic-addon11">Add Heading Text</label>
                        </div>
                    </div>
                </div>
                <div class="col-lg-4  mt-3">
                    <div class="input-group input-group-merge">
                        ...
                        <div class="form-floating form-floating-outline">
                            ...
                            <label for="basic-addon11">Subheading Text</label>
                        </div>
                    </div>
                </div>
                <div class="col-lg-12  mt-3">
                    <div class="input-group input-group-merge">
                        <span class="input-group-text">@</span>
                        <div class="form-floating form-floating-outline">
                            <input type="text" class="form-control" name="des1" placeholder="Link" aria-label="Link" value="<?php echo $des1; ?>" />
                    ...
                    <div class="card mb-4">
                        ...
                        <div class="card-body demo-vertical-spacing demo-only-element z4">
                            <div class="row">
                                <?php
                                $pid = $_GET['pid'];
                                $productdata = sqlfetch("SELECT * FROM `client` where id='$pid' ");
                ...
                <div class="card-datatable dataTable_select text-nowrap table-responsive">
                    <table id="tableID" class="display dt-select-table table table-bordered" style="width:100%">
                        ...
                        <tbody>
                            <?php
                            $count = 1;
                            $data = sqlfetch("SELECT * FROM `client`  order by fld_order");
                            foreach ($data as $menu) { ?>
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <a href="banner.php?id=<?php echo $menu['id']; ?>&action=delete" onclick="return confirm('Are you sure you want to delete this item?');">
