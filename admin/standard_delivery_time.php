...
if (isset($_POST['addclient'])) {
    ...
    $type = 1;
    $pdo = getPDOObject();
    // $sql = $pdo->query("SELECT * FROM `standard_delivery_time` where  name LIKE '$name'");
    // $num = $sql->rowCount();
    $num = 0;
    ...
    if (!$num) {
        ...
        $q = $pdo->prepare("INSERT into `standard_delivery_time` values(:id,:name, :photo,:des,:des1, :fld_order, :actstat)");
        $q->execute(array(
            ':id' => $id,
...
if ($_GET['action'] == 'delete') {
    $sql_del = sqlfetch("delete from standard_delivery_time where id='" . $_GET['id'] . "'");
    echo "<script>window.open('standard_delivery_time.php','_self')</script>";
}
...
if (isset($_POST['deleteall'])) {
    ...
    if (count($arr)) {
        ...
        $data = sqlfetch("select * from `standard_delivery_time` where id in ($str_rest_refs)");
        ...
        $pdo = getPDOObject();
        $q = $pdo->query("DELETE FROM `standard_delivery_time` WHERE id in ($str_rest_refs)");
        if ($q)
            $umessage = '<div class="alert alert-success" role="alert">
...
if (isset($_POST['editdone'])) {
    ...
    $pdo = getPDOObject();
    $q = $pdo->prepare("UPDATE `standard_delivery_time` SET 
...
{ ?>
    <form action="standard_delivery_time.php" method="post" enctype="multipart/form-data">
                ...
                <div class="col-lg-4  mt-3" style="display: none;">
                    <div class="input-group input-group-merge">
                        ...
                        <div class="form-floating form-floating-outline">
                            ...
                            <label for="basic-addon11">Add Link</label>
                        </div>
                    </div>
                </div>
...
<div class="content-wrapper">
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <h4 class="py-3 mb-4"><span class="text-muted fw-light">Home/</span>Banners</h4>
                    ...
                    <div class="card mb-4">
                        ...
                        <div class="card-body demo-vertical-spacing demo-only-element z4">
                            <div class="row">
                                <?php
                                $pid = $_GET['pid'];
                                $productdata = sqlfetch("SELECT * FROM `standard_delivery_time` where id='$pid' ");
                ...
                <div class="card-datatable dataTable_select text-nowrap table-responsive">
                    <table id="tableID" class="display dt-select-table table table-bordered" style="width:100%">
                        ...
                        <tbody>
                            <?php
                            $count = 1;
                            $data = sqlfetch("SELECT * FROM `standard_delivery_time`  order by fld_order");
                            foreach ($data as $menu) { ?>
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <a class="ajax-link" href="standard_delivery_time.php?&pid=<?php echo $menu['id']; ?>&edit=true">
                                    ...
                                    <td>
                                        ...
                                        <a href="standard_delivery_time.php?id=<?php echo $menu['id']; ?>&action=delete" onclick="return confirm('Are you sure you want to delete this item?');">
