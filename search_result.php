...
?>
            ...
            if (!empty($media['photo']) && file_exists($imagePath)) {
                $displayImg = SITE_URL . $imagePath;
            }
            else {
                $placeholderIndex = $media['id'] % count($media_placeholders);
                $displayImg = SITE_URL . "upload/" . $media_placeholders[$placeholderIndex];
            }
?>
