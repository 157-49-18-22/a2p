...
if (isset($_POST['addclient'])) {
    ...
    $type = 1;
    $pdo = getPDOObject();
    // $sql = $pdo->query("SELECT * FROM `fixed_delivery_time` where  name LIKE '$name'");
    // $num = $sql->rowCount();
    $num = 0;
    ...
    if (!$num) {
        ...
        $q = $pdo->prepare("INSERT into `fixed_delivery_time` values(:id,:name, :photo,:des,:des1, :meta_title, :meta_keyword, :meta_description, :fld_order, :actstat)");
        $q->execute(array(
            ':id' => $id,
...
if ($_GET['action'] == 'delete') {
    $sql_del = sqlfetch("delete from fixed_delivery_time where id='" . $_GET['id'] . "'");
    echo "<script>window.open('fixed_delivery_time.php','_self')</script>";
}
...
if (isset($_POST['deleteall'])) {
    ...
    if (count($arr)) {
        ...
        $data = sqlfetch("select * from `fixed_delivery_time` where id in ($str_rest_refs)");
        ...
        $pdo = getPDOObject();
        $q = $pdo->query("DELETE FROM `fixed_delivery_time` WHERE id in ($str_rest_refs)");
        if ($q)
            $umessage = '<div class="alert alert-success" role="alert">
...
if (isset($_POST['editdone'])) {
    ...
    $pdo = getPDOObject();
    $q = $pdo->prepare("UPDATE `fixed_delivery_time` SET 
       ...
       meta_description=?,
		...
		WHERE id=?");
    $q->execute(array($name, $Filename, $des,  $des1, $meta_title, $meta_keyword, $meta_description,   $fld_order, $actstat, $pid));
    $affected_rows = $q->rowCount();
    if ($affected_rows)
...
function client_form($pid = '0', $name = '', $photo = '', $des = '', $des1 = '',  $meta_title = '', $meta_keyword = '', $meta_description = '', $fld_order = '0', $actstat = '', $formname = 'addclient')
{ ?>
    <form action="fixed_delivery_time.php" method="post" enctype="multipart/form-data">
                ...
                <div class="col-lg-4  mt-3"  >
                    <div class="input-group input-group-merge">
                        ...
                        <div class="form-floating form-floating-outline">
                            ...
                            <label for="basic-addon11">Youtube Frame</label>
                        </div>
                    </div>
                </div>
...
<div class="content-wrapper">
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <h4 class="py-3 mb-4"><span class="text-muted fw-light">Home/</span>Fixed Time Delivery </h4>
                    ...
                    <div class="card mb-4">
                        ...
                        <div class="card-body demo-vertical-spacing demo-only-element z4">
                            <div class="row">
                                <?php
                                $pid = $_GET['pid'];
                                $productdata = sqlfetch("SELECT * FROM `fixed_delivery_time` where id='$pid' ");
                                foreach ($productdata as $product) {
                                    extract($product);
                                    client_form($pid, $name, $photo, $des, $des1,  $meta_title, $meta_keyword, $meta_description,  $fld_order, $actstat, $formname = 'editdone');
                                } ?>
                            </div>
                        </div>
                    </div>
                ...
                <div class="card-datatable dataTable_select text-nowrap table-responsive">
                    <table id="tableID" class="display dt-select-table table table-bordered" style="width:100%">
                        ...
                        <tbody>
                            <?php
                            $count = 1;
                            $data = sqlfetch("SELECT * FROM `fixed_delivery_time`  order by fld_order");
                                ...
                                <tr>
                                   ...
                                   <td><img style="height: 32px;width: 32px;" src="../upload/<?php echo $menu['photo']; ?>" class="img-responsive"></td>
                                    ...
                                    <td>
                                        ...
                                        <a class="ajax-link" href="fixed_delivery_time.php?&pid=<?php echo $menu['id']; ?>&edit=true">
                                    ...
                                    <td>
                                        ...
                                        <a href="fixed_delivery_time.php?id=<?php echo $menu['id']; ?>&action=delete" onclick="return confirm('Are you sure you want to delete this item?');">
