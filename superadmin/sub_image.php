...
if (isset($_POST['addsubcategory'])) {
    ...
    if (!$num) {
        ...
        $affected_rows = insert('sub_image', $posted_data);
        if ($affected_rows)
            if ($affected_rows)
...
if ($_GET['action'] == 'delete') {
    $sql_del = sqlfetch("delete from sub_image where id='" . $_GET['id'] . "'");
    echo "<script>window.open('sub_image','_self')</script>";
}
...
if (isset($_POST['deleteall'])) {
    ...
    if (count($arr)) {
        ...
        $data = sqlfetch("select * from `sub_image` where id in ($str_rest_refs)");
        ...
        $pdo = getPDOObject();
        $q = $pdo->query("DELETE FROM `sub_image` WHERE id in ($str_rest_refs)");
        if ($q)
            $umessage = '<div class="alert alert-success" role="alert">
...
if (isset($_POST['activate'])) {
    ...
    if (count($arr)) {
        $str_rest_refs = implode(",", $arr);
        $pdo = getPDOObject();
        $q = $pdo->query("UPDATE `sub_image` SET actstat='1' WHERE id in ($str_rest_refs)");
        if ($q)
            $umessage = '<div class="alert alert-success" role="alert">
...
if (isset($_POST['deactivate'])) {
    ...
    if (count($arr)) {
        $str_rest_refs = implode(",", $arr);
        $pdo = getPDOObject();
        $q = $pdo->query("UPDATE `sub_image` SET actstat='0' WHERE id in ($str_rest_refs)");
        if ($q)
            $umessage = '<div class="alert alert-success" role="alert">
...
if (isset($_POST['editdone'])) {
    ...
    $affected_rows = update('sub_image', $posted_data, array('id' => $pid));
    if ($affected_rows)
        $umessage = '<div class="alert alert-success" role="alert">
...
{ ?>
    ...
    <form action="sub_image.php" method="post" enctype="multipart/form-data">
                    ...
                    <div class="card mb-4">
                        ...
                        <div class="card-body demo-vertical-spacing demo-only-element z4">
                            <div class="row">
                                <?php
                                $pid = $_GET['pid'];
                                $productdata = sqlfetch("SELECT * FROM `sub_image` where id='$pid' ");
                ...
                <div class="card-datatable dataTable_select text-nowrap table-responsive">
                    <table id="tableID" class="display dt-select-table table table-bordered" style="width:100%">
                        ...
                        <tbody>
                            ...
                            $data = sqlfetch("SELECT * FROM `sub_image` ORDER BY fld_order");
                            ...
                            ?>
                                <tr>
                                    ...
                                    <td>
                                        <a class="ajax-link" href="sub_image.php?pid=<?php echo $menu['id']; ?>&edit=true">
                                            <button type="button" class="btn btn-xs btn-danger pull-right" name="editclient"><i class="fa fa-pencil"></i></button>
                                        </a>
                                    </td>

                                    <td>
                                        <a href="sub_image.php?id=<?php echo $menu['id']; ?>&action=delete" onclick="return confirm('Are you sure you want to delete this item?');">
