...
if (isset($_POST['addsubcategory'])) {
    ...
    // Ensure that permissions are saved as a JSON string
    if (isset($posted_data['permissions'])) {
        $posted_data['permissions'] = json_encode($posted_data['permissions']);
    } else {
        $posted_data['permissions'] = json_encode([]); // Ensure it's stored as JSON even if empty
    }

    // Checking for duplicate entries (if required)
    $num = 0;

    // Insert the new subadmin data without handling any image
    if (!$num) {
...
if (isset($_POST['editdone'])) {
    ...
    // Update the subadmin data without handling any image
    $affected_rows = update('subadmin', $posted_data, array('id' => $pid));
    if ($affected_rows) {
        $umessage = '<div class="alert alert-success" role="alert">
...
function subcategory_form($pid = '0', $name = '', $bio = '',  $username = '', $password = '', $permissions = [], $formname = 'addsubcategory')
{
    // Ensure $permissions is an array
    if (empty($permissions)) {
        $permissions = []; // Set as an empty array if it's null or not set
    } else {
        $permissions = json_decode($permissions, true); // Decode JSON to array if it's a JSON string
    }
?>
            ...
            <div class="row">
                <div class="col-lg-12 mt-3">
                    <div class="row">
                        ...
                        <?php if ($pid > 0) { // Only show permissions if in edit mode ?>
                        <div class="col-md-12 mt-4">
                            <div class="form-floating form-floating-outline">
                                ...
                                <label for="select2Multiple">Give Permissions</label>
                            </div>
                        </div>
                        <?php } ?>
                        ...
                        <div class="card-body demo-vertical-spacing demo-only-element z4">
                            ...
                            <?php
                                ...
                                foreach ($productdata as $product) {
                                    extract($product);
                                    subcategory_form($pid, $name, $bio,  $username, $password, $permissions,  $formname = 'editdone');
                                } ?>
                            </div>
                        </div>
                    </div>
