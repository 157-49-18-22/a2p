...
if (isset($_POST['addclient'])) {
    ...
    $type = 1;
    $pdo = getPDOObject();
    // $sql = $pdo->query("SELECT * FROM `news` where  name LIKE '$name'");
    // $num = $sql->rowCount();
    $num = 0;
    ...
    if (!$num) {
        ...
        $q = $pdo->prepare("INSERT into `news` values(:id,:name, :photo,:des,:des1, :slug, :fld_order, :actstat)");
        $q->execute(array(
            ':id' => $id,
...
if ($_GET['action'] == 'delete') {
    $sql_del = sqlfetch("delete from news where id='" . $_GET['id'] . "'");
...
if (isset($_POST['deleteall'])) {
    ...
    if (count($arr)) {
        ...
        $data = sqlfetch("select * from `news` where id in ($str_rest_refs)");
        ...
        $pdo = getPDOObject();
        $q = $pdo->query("DELETE FROM `news` WHERE id in ($str_rest_refs)");
        if ($q)
            $umessage = '<div class="alert alert-success" role="alert">
                    ...
                    <div class="card mb-4">
                        ...
                        <div class="card-body demo-vertical-spacing demo-only-element z4">
                            <div class="row">
                                <?php
                                $pid = $_GET['pid'];
                                $productdata = sqlfetch("SELECT * FROM `news` where id='$pid' ");
                ...
                <div class="card-datatable dataTable_select text-nowrap table-responsive">
                    <table id="tableID" class="display dt-select-table table table-bordered" style="width:100%">
                        ...
                        <tbody>
                            <?php
                            $count = 1;
                            $data = sqlfetch("SELECT * FROM `news`  order by fld_order");
                            foreach ($data as $menu) { ?>
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <a class="ajax-link" href="news.php?&pid=<?php echo $menu['id']; ?>&edit=true">
                                    ...
                                    <td>
                                        ...
                                        <a href="news.php?id=<?php echo $menu['id']; ?>&action=delete" onclick="return confirm('Are you sure you want to delete this item?');">
